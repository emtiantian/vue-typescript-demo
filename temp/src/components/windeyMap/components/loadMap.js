...
const loadMap = (mapKey, autoChange, allowTime = 2, intervals = 200) => {
    if (mapKey.length > 0) {
        // 排序
        mapKey.sort((a, b) => {
 ...
 * @param allowTime
 */
const createApi = async (mapKey, autoChange, zoom, name, center, intervals = 200, allowTime = 2) => {
    // 引入google地图api或者百度地图aip 返回一个promise对象
    console.log('开始加载地图');
    const value = await loadMap(mapKey, autoChange, allowTime, intervals);
    console.log('加载地图完成');
